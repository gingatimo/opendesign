...
// chính giữa viewport. Thuần TRÌNH BÀY — consumer tự dựng card & đặt bề rộng (nên < viewport để lộ phần
// peek hai bên). Khác GCarousel (free-scroll, mọi item ngang hàng).
@Component({
...
export class GCoverflow {
  ...
  protected readonly canPrev = computed(() => this.active() > 0);
  protected readonly canNext = computed(() => this.active() < this.count() - 1);
  ...
  protected move(dir: 1 | -1): void {
    ...
    this.active.set(Math.min(Math.max(this.active() + dir, 0), Math.max(n - 1, 0)));
  }
