...
@Component({
  imports: [GSelect, GOption, ReactiveFormsModule],
  template: `
    <g-select [formControl]="control" searchable placeholder="Chọn quả">
      <g-option value="apple">Táo</g-option>
      <g-option value="orange">Cam</g-option>
      <g-option value="banana">Chuối</g-option>
    </g-select>
  `,
})
...
describe('GSelect searchable', () => {
  ...
  function optionEls(fixture: ReturnType<typeof open>['fixture']) {
    return fixture.debugElement
      .queryAll(By.directive(GOption))
      .map((d) => d.nativeElement as HTMLElement);
  }
  function typeSearch(fixture: ReturnType<typeof open>['fixture'], text: string) {
    const input = fixture.debugElement.query(By.css('.g-select__search'))
  ...
  it('mở panel hiện ô tìm kiếm', () => {
    const { fixture } = open();
    expect(fixture.debugElement.query(By.css('.g-select__search'))).not.toBeNull();
  });

  it('gõ "cam" chỉ hiện option Cam (ẩn còn lại)', () => {
    const { fixture } = open();
    typeSearch(fixture, 'cam');
    const opts = optionEls(fixture);
    const hidden = opts.map((o) => o.classList.contains('g-option--hidden'));
    // Táo ẩn, Cam hiện, Chuối ẩn
    expect(hidden).toEqual([true, false, true]);
  });

  it('tìm không dấu-nhạy: "chuoi" khớp "Chuối"', () => {
    const { fixture } = open();
    typeSearch(fixture, 'chuoi');
    const opts = optionEls(fixture);
    expect(opts[2].classList.contains('g-option--hidden')).toBe(false);
    expect(opts[0].classList.contains('g-option--hidden')).toBe(true);
  });

  it('không khớp gì → hiện empty state', () => {
    const { fixture } = open();
    typeSearch(fixture, 'xyz');
    expect(fixture.debugElement.query(By.css('.g-select__empty'))).not.toBeNull();
  });
});
...
@Component({
  imports: [GSelect, GOption, ReactiveFormsModule],
  template: `
    <g-select [formControl]="control" multiple placeholder="Chọn nhiều">
      <g-option value="a">A</g-option>
      <g-option value="b">B</g-option>
      <g-option value="c">C</g-option>
    </g-select>
  `,
})
class MultipleHost {
...
describe('GSelect multiple', () => {
  ...
  function optionEls(fixture: ReturnType<typeof setup>['fixture']) {
    return fixture.debugElement
      .queryAll(By.directive(GOption))
      .map((d) => d.nativeElement as HTMLElement);
  }

  it('chọn nhiều: value là mảng, panel GIỮ mở, toggle bỏ chọn', () => {
    const { fixture, trigger } = setup();
    const opts = optionEls(fixture);
    opts[0].click();
    fixture.detectChanges();
    expect(fixture.componentInstance.control.value).toEqual(['a']);
    expect(trigger.getAttribute('aria-expanded')).toBe('true'); // panel còn mở
    opts[1].click();
    fixture.detectChanges();
    expect(fixture.componentInstance.control.value).toEqual(['a', 'b']);
    opts[0].click(); // toggle bỏ A
    fixture.detectChanges();
    expect(fixture.componentInstance.control.value).toEqual(['b']);
  });

  it('option đã chọn có aria-selected + dấu check', () => {
    const { fixture } = setup();
    const opts = optionEls(fixture);
    opts[0].click();
    fixture.detectChanges();
